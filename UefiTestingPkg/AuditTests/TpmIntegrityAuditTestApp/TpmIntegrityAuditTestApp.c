...
Audit test to Check the integrity of TPM Event Logs by Comparing PCR Digest Values from TPM Device with Digest value from Event Log Memory
...
#define SHA256_STR_LEN  96                       // To Store 62 Digest Characters along with Separated Space
#define SHA_BUF_LEN   3                        // To Store Hex value in 02x format including '\0'
    ...
    )
{
  ...
  //Allocate context buffer size required for SHA-256 hash operations
  ...
  while ((UINTN)TcgPcrEvent2 <= EventEndAddr) {
    ...
    for (DigestIndex = 0; DigestIndex < DigestCount; DigestIndex++) {
      ...
      if (TcgPcrEvent2->PCRIndex < MAX_PRINT_PCR_INDEX) {
        ...
        if(EFI_ERROR(Status)){
          Print(L"Sha256Update Failed Status:%r\n",Status);
          FreePool(ShaCtx);
          return Status;
        }
        ...
        if(EFI_ERROR(Status)){
          Print(L"Sha256Update Digest on DataBuffer Failed Status:%r\n",Status);
          FreePool(ShaCtx);
          return Status;
        }
        ...
        if(EFI_ERROR(Status)){
          Print(L"Sha256Final Failed Status:%r\n",Status);
          FreePool(ShaCtx);
          return Status;
        }
        // Update the current PCR value
        CopyMem(g_CurrentPcrValues[TcgPcrEvent2->PCRIndex], NewPcrValue, SHA256_DIGEST_SIZE);

      } 
      // Move DigestBuffer forward by DigestSize
  ...
  )
{
  ...
  // Print Final EventShaData and PCR Data
  ...
  ConvertEventToString(PcrIndex, PcrValues,EventString[PcrIndex],PcrString[PcrIndex]);
  ...
  )
{
 ...
 // ZeroMem(EventString, sizeof(EventString));
  ...
  AddTestCase (TpmIntegrity, "PCR0 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest0);
  AddTestCase (TpmIntegrity, "PCR1 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest1);
  AddTestCase (TpmIntegrity, "PCR2 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest2);
  AddTestCase (TpmIntegrity, "PCR3 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest3);
  AddTestCase (TpmIntegrity, "PCR4 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest4);
  AddTestCase (TpmIntegrity, "PCR5 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest5);
  AddTestCase (TpmIntegrity, "PCR6 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest6);
  AddTestCase (TpmIntegrity, "PCR7 IntegrityCheck", "TpmIntegrityTest", CheckTPMLogIntegrity , NULL, CleanUpTestContext, &PcrTest7);
